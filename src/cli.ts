...
const USAGE = `friendly-ukrainization — decline Ukrainian words from the command line
...
function main(): void {
  ...
  switch (command) {
    case 'decline': {
      ...
      console.log(caseName ? decline(word, caseName, options) : JSON.stringify(declineAll(word, options), null, 2));
      break;
    }
    case 'numeral': {
      const [countStr] = positional;
      const count = Number(countStr);
      if (!countStr || !Number.isFinite(count)) fail('numeral requires a whole number, e.g. `friendly-ukrainization numeral 21000 --case genitive`');
      if (caseName) {
        console.log(declineNumeralWord(count, caseName, gender, animacy));
      } else {
    ...
    case 'adjective': {
      const [word] = positional;
      if (!word) fail('adjective requires a masculine nominative singular word, e.g. `friendly-ukrainization adjective молодий --gender feminine`');
    ...
    case 'name': {
      if (positional.length < 2) fail('name requires at least a firstName and lastName, e.g. `friendly-ukrainization name Дмитро Ковальчук --case genitive`');
      ...
      console.log(
        caseName ? declineName(name, caseName, gender).full : JSON.stringify(declineNameAll(name, gender), null, 2),
      );
      break;
    }
    case 'transliterate': {
      if (positional.length === 0) fail('transliterate requires text, e.g. `friendly-ukrainization transliterate Дмитро Ковальчук`');
      console.log(transliterate(positional.join(' ')));
      break;
    }
    default:
